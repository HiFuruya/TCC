<x-app-layout>
    ...
    <div align="right">
        <a  href="{{ route('produtos_transacao.create', $nota[0]->id) }}" class="btn btn-primary">
                    ...
                    @foreach ($transacoes as $item)
                        <tr>
                            ...
                            <td class="d-none d-md-table-cell">{{ ($item->valor_total)}}</td>
                                <td>
                                    <a nohref style="cursor:pointer" onclick="showInfoModal(['PRODUTO: {{$item->produto->nome}}', 'PLANTAÇÃO: {{$item->plantacao->nome}}', 'QUANTIDADE: {{$item->quantidade}}', 'VALOR UNITÁRIO: {{$item->valor_unitario}}', 'DESCONTO: {{$item->desconto}}', 'TOTAL: {{$item->valor_total}}'])" class="btn btn-primary">
