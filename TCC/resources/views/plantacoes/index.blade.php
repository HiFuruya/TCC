<x-app-layout>
    ...
    <div align="right">
        <a  href="{{ route('plantacoes.create') }}" class="btn btn-primary">Adicionar
                    ...
                    @foreach ($plantacoes as $item)
                        <tr>
                            ...
                            <td class="d-none d-md-table-cell">{{ $item->liquido }}</td>
                                <td>
                                    <button class="d-md-none btn btn-primary" nohref style= "cursor:pointer" onclick="showInfoModal(['PLANTAÇÃO: {{$item->nome}}', 'PLANTIO: {{$item->plantio}}', 'TIPO: {{$item->planta->nome}}', 'LUA DO PLANTIO: {{$item->lua}}', 'MUDAS: {{$item->mudas}}', 'GANHOS: {{$item->lucro}}', 'GASTOS: {{$item->gasto}}', 'LÍQUIDO: {{$item->liquido}}'])" class="btn btn-primary">
